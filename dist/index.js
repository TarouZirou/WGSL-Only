...
const pipeline = g_dev.createRenderPipeline({
    layout: "auto",
    vertex: {
...
function render(ctx, pipeline) {
    ...
    g_dev.queue.writeBuffer(uniBuf, 4 * 2, new Float32Array([mouse.x, mouse.y]));
    g_dev.queue.writeBuffer(uniBuf, 4 * 4, new Float32Array([canvas.width, canvas.height]));
